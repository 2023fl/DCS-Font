...
def get_parser():
    ...
    #                     help="Path to the checkpoint to resume training from. If None, start training from scratch.")
    # parser.add_argument("--resume_global_step", type=int, default=,
    #                     help="Global step to resume training from. If None, will be inferred from the checkpoint.")
    ...
    # parser.add_argument("--data_root", type=str, default='D:/FILES/Project/pytorch/Github/FontDiffuser',
    #                     help="The font dataset root path.",)
    # parser.add_argument("--output_dir", type=str, default=None,
    #                     help="The output directory where the model predictions and checkpoints will be written.")  # 原
    parser.add_argument("--output_dir", type=str, default='FontDiffuser/output',
                        help="The output directory where the model predictions and checkpoints will be written.")    # 改
    ...
    parser.add_argument("--num_ref", type=int, default=3,
                        help="number of reference glyphs for few-shot style encoding")
    ...
    parser.add_argument("--phase_2", action="store_true", help="Training in phase 2 using SCR module.")  # action="store_true": 这个参数指定了当命令行中出现--phase_2时应该执行的动作。store_true意味着如果这个参数被指定了（即，如果--phase_2出现在命令行中），则对应的值会被设置为True。如果这个参数没有被指定，则默认为False。这种参数常用于开关选项，即只需要知道这个选项是否被激活，而不需要知道它的具体值。
    parser.add_argument("--phase_1_ckpt_dir", type=str, default='phase_1_ckpt/global_step_120000', help="The trained ckpt directory during phase 1.")
    #    phase_1_ckpt_dir就是D:\FILES\Project\pytorch\Github\FontDiffuser\myfile_mane
    # SCR
    ...
    parser.add_argument("--scr_ckpt_path", type=str, default=r"ckpt_dir/scr_210000.pth")    # 'D:/FILES/Project/pytorch/Github/FontDiffuser/ckpt_dir/scr_210000.pth'
