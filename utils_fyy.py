...
def save_single_image(save_dir, image, filename="generated.png"):
    """Save a single image with custom filename"""
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    if isinstance(image, torch.Tensor):
        image = image.cpu().numpy()
        image = (image.transpose(1, 2, 0) + 1.0) * 127.5
...
def save_image_with_content_style(save_dir, image, content_image_pil,
                                  content_image_path, style_image_path,
                                  resolution, filename="out_with_cs.jpg"):
    """Save combined image with custom filename"""
    new_image = Image.new('RGB', (resolution * 3, resolution))

    # Handle content image
    if content_image_pil is not None:
        content_image = content_image_pil.resize((resolution, resolution), Image.BILINEAR)
    else:
        content_image = Image.open(content_image_path).convert("RGB").resize((resolution, resolution), Image.BILINEAR)
...
def reNormalize_img(pred_original_sample):
    return (pred_original_sample / 2 + 0.5).clamp(0, 1)


def normalize_mean_std(image):
    transforms_norm = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    return transforms_norm(image)
...
def load_ttf(ttf_path, fsize=85):
    pygame.init()
    font = pygame.freetype.Font(ttf_path, size=fsize)
    return font


def ttf2im(font, char, fsize=100):
    try:
        surface, _ = font.render(char)
    except:
        print(f"No glyph for char {char}")
        return None
    ...
    try:
        ...
        print(f"Error processing char {char}: {str(e)}")
        return None
