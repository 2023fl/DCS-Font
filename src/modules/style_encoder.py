...
class StyleSEAttention(nn.Module):
    def __init__(self, channel=512, reduction=8):
        super(StyleSEAttention, self).__init__()
...
class StyleEncoder(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.arch = style_encoder_textedit_addskip_arch(
            self.ch,
            self.out_channel_multiplier,
            input_nc
        )[self.resolution]

        self.se_attentions = nn.ModuleList([
            StyleSEAttention(channel=out_channel)
            for out_channel in self.arch['out_channels']
        ])
    ...
    def init_weights(self):
        ...
        print('Param count for FewShotStyleEncoder initialized parameters: %d' % self.param_count)
    ...
    def encode_single(self, x):
        ...
        for index, blocklist in enumerate(self.blocks):
            ...
            if index in self.save_features[:-1]:
                residual_features.append(h)
            h = self.se_attentions[index](h)

        style_emd = h
        h_vec = F.adaptive_avg_pool2d(h, (1, 1))
        h_vec = h_vec.view(h_vec.size(0), -1)

        return style_emd, h_vec, residual_features
    ...
    def forward(self, x):
        if x.dim() == 4:
            ...
            style_emd_flat, h_flat, residuals_flat = self.encode_single(x_flat)
            se_shape = style_emd_flat.shape
            style_emd = style_emd_flat.view(B, N, se_shape[1], se_shape[2], se_shape[3])
            h_dim = h_flat.shape[1]
            h = h_flat.view(B, N, h_dim)
            residual_features = []
            for rf in residuals_flat:
                rf_shape = rf.shape
                rf_reshaped = rf.view(B, N, rf_shape[1], rf_shape[2], rf_shape[3])
                residual_features.append(rf_reshaped)

            fused_style_emd = style_emd.mean(dim=1)
            fused_h = h.mean(dim=1)

            fused_residuals = []
            for rf in residual_features:
                fused_residuals.append(rf.mean(dim=1))

            print('few')
            return fused_style_emd, fused_h, fused_residuals

        else:
            raise ValueError("Unsupported input dimensions for FewShotStyleEncoder.forward: got x.dim() = {}".format(x.dim()))
