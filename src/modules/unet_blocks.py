...
class UNetMidMCABlock2D(nn.Module):
    def __init__(
        ...
        self.safm = SAFM(in_channels)

    def forward(self, hidden_states, temb=None, encoder_hidden_states=None, index=None):
        ...
        hidden_states = self.safm(hidden_states)
...
class StyleRSIUpBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, style_structure_features, temb=None, encoder_hidden_states=None, upsample_size=None):
        total_offset = 0
        style_content_feat = style_structure_features[-self.upblock_index - 2]

        for i, (sc_inter_offset, dcn_deform, resnet, attn) in enumerate(zip(self.sc_interpreter_offsets, self.dcn_deforms, self.resnets, self.attentions)):
