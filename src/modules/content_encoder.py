...
class ContentSEAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        scale = self.fc(y)  # Shape: [b, c]

        residual = self.residual(y)  # Shape: [b, c]
        scale = scale + residual

        scale = self.sigmoid(scale).view(b, c, 1, 1)  # Shape: [b, c, 1, 1]
...
class ContentEncoder(ModelMixin, ConfigMixin):
    ...
    def __init__(self, G_ch=64, G_wide=True, resolution=128,
        ...
        self.se_attentions = nn.ModuleList([
            ContentSEAttention(channel=out_channel)
    ...
    def forward(self, x, return_attn=False):
        ...
        for index, blocklist in enumerate(self.blocks):
            ...
            if return_attn:
                ...
                b, c, _, _ = h.size()
                attn = se_module.sigmoid(se_module.fc(se_module.avg_pool(h).view(b, c))).detach().cpu().numpy()
